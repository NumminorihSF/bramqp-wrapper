  ...
  bramqp.initialize(this.socket, this.protocolVersion, (error, handle) => {
    ...
    handle.openAMQPCommunication(this.user, this.password, true, this.vhost || '/', (error) => {
      ...
      (new ChannelConstructor(handle, 1)).close((err) => {
        ...
        this.taskPool.setChannelPool(new ChannelPool(handle, ChannelConstructor, this.channelPoolCapacity));
        callback();
      });
    });
  });
...
/**
 * Get channel form channel pool. Make channel busy, but dos not increment counter of tasks in work.
