...
Client.prototype._reconnect = function(){
  ...
  this.reconnectTimeout = setTimeout(()=>{
    this.connect((err) => {
      if (err){
        this.emit('error', new Error('Reconnect Error: '+err.message));
        this._reconnect();
      }
    });
 ...
 *    3rd argument is done callback to make channel free.
 */
Client.prototype.getChannelOnConnect = function(callback){
  if (this.should_work) return this.taskPool.getChannel(callback);
  if (this.connected) return this.taskPool.getChannel(callback);
  return this.once('connect', ()=>{
    setImmediate(()=>{this.taskPool.getChannel()});
  });
};
